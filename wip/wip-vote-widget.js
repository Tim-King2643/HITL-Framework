...
(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = [
      ...
      '.wip-status-badge{display:inline-block;font-size:10.5px;font-weight:700;letter-spacing:.03em;text-transform:uppercase;',
      ...
      '.wip-needs-review{box-sizing:border-box;max-width:860px;margin:0 auto 24px;padding:16px 22px;',
      'border-radius:10px;border:1px solid #F0C67A;border-left:5px solid #B8873F;background:#FEF3C7;',
      'font-family:"Segoe UI",sans-serif;}',
      '.wip-needs-review .wnr-head{display:flex;align-items:center;gap:10px;font-size:14px;font-weight:700;color:#7A5A1E;margin-bottom:8px;}',
      '.wip-needs-review .wnr-count{display:inline-flex;align-items:center;justify-content:center;min-width:22px;height:22px;',
      'padding:0 6px;border-radius:11px;background:#B8873F;color:#fff;font-size:12px;font-weight:700;}',
      '.wip-needs-review ul{margin:0;padding-left:20px;}',
      '.wip-needs-review li{font-size:13px;color:#5A4315;line-height:1.7;}',
      '.wip-needs-review a{color:#7A5A1E;font-weight:700;text-decoration:none;}',
      '.wip-needs-review a:hover{text-decoration:underline;}',
      '.wip-needs-review .wnr-clear{font-size:13px;color:#1B7A4C;font-weight:600;}',
      '.item-row.wip-row-needs-action{box-shadow:inset 4px 0 0 #B8873F;}'
    ].join('');
  ...
  // Decisions where the item is still actively awaiting a reviewer's call —
  // settled states (approve, deny, park, pending-publish, published) don't count.
  var ACTIONABLE_DECISIONS = { pending: true, revise: true, discuss: true };

  function itemLinkFromRow(tr) {
    var htmlLink = tr.querySelector('.fmt-links a[href$=".html"]');
    if (htmlLink) return { href: htmlLink.getAttribute('href'), text: tr.querySelector('.item-title a') ? tr.querySelector('.item-title a').textContent : htmlLink.textContent };
    var titleLink = tr.querySelector('.item-title a');
    if (titleLink) return { href: titleLink.getAttribute('href'), text: titleLink.textContent };
    return null;
  }

  function renderNeedsReview() {
    var mount = document.querySelector('[data-wip-needs-review-mount]');
    if (!mount) return;
    var statusSpans = document.querySelectorAll('[data-wip-status]');
    if (!statusSpans.length) { mount.innerHTML = ''; return; }

    var pending = [];
    statusSpans.forEach(function (span) {
      var itemId = span.getAttribute('data-wip-status');
      var itemData = latestItems[itemId];
      if (!itemData || !ACTIONABLE_DECISIONS[itemData.decision]) return;
      var tr = span.closest('tr');
      // Only count it as "needs your review" when you specifically haven't voted yet —
      // if you already have (any choice), it's not on you, even while still pending overall.
      var myVote = identity === 'Tim' ? itemData.tim : identity === 'GiGi' ? itemData.gigi : null;
      if (myVote) return;
      if (tr) tr.classList.add('wip-row-needs-action');
      var link = tr ? itemLinkFromRow(tr) : null;
      pending.push({ itemId: itemId, link: link });
    });

    if (!identity) { mount.innerHTML = ''; return; }

    if (!pending.length) {
      mount.innerHTML = '<div class="wip-needs-review"><div class="wnr-head" style="color:#1B7A4C;">' +
        '<span class="wnr-clear">Nothing waiting on you, ' + escapeHtml(identity) + ' — you\'re caught up.</span></div></div>';
      return;
    }

    var items = pending.map(function (p) {
      var label = p.link ? escapeHtml(p.link.text) : escapeHtml(p.itemId);
      return p.link ? '<li><a href="' + escapeHtml(p.link.href) + '">' + label + '</a></li>' : '<li>' + label + '</li>';
    }).join('');

    mount.innerHTML = '<div class="wip-needs-review">' +
      '<div class="wnr-head"><span class="wnr-count">' + pending.length + '</span> ' +
      'waiting on ' + escapeHtml(identity) + '’s review</div>' +
      '<ul>' + items + '</ul></div>';
  }
